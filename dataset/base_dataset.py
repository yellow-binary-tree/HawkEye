        ...
        for _ in range(self.num_tries):
            try:
                ...
                frames, frame_indices, fps = self.video_reader(
                    ...
                    max_num_frames=max_num_frames, client=self.client, clip=clip, fps=getattr(self, "fps", None)
                )
            ...
            if return_fps:
                assert fps is not None or hasattr(self, 'fps')
                fps = fps if fps is not None else self.fps   # for some case that fps cannot be inferred from the video, we can set it manually
