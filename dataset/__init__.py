...
def create_dataset(dataset_type, config):
    ...
    if dataset_type == "pt_train":
        ...
        train_media_types = sorted(list({e['media_type'] for e in train_files}))
        ...
        for m in train_media_types:
            ...
            _train_files = [e for e in train_files if e['media_type'] == m]

            datasets = []
            for train_file in _train_files:
                dataset_kwargs = train_file.copy()
        ...
        train_datasets = []
        every_single_dataset = []

        for m in train_media_types:
            dataset_cls = ITImgTrainDataset if m == "image" else ITVidTrainDataset
            # dataset_cls = {"image" : ITImgTrainDataset, "video": MyITVidTrainDataset, "video_truncate": MyITVidTruncateTrainDataset}[m]
            # dataset of the same media_type will be mixed in a single Dataset object
            _train_files = [e for e in train_files if e['media_type'] == m]
            ...
            for train_file in _train_files:
                ...
                if "video" in m:
                    video_only_dataset_kwargs_train.update({
                        ...
                        "add_second_msg": config.model.get("add_second_msg", True),
                        "grounding_method": train_file.get("grounding_method", None),
                        "min_gold_clips": train_file.get("min_gold_clips", 1),
                        "max_gold_clips": train_file.get("max_gold_clips", None),
                        "num_examples": train_file.get("num_examples", None),
                    })
                    dataset_kwargs.update(video_only_dataset_kwargs_train)
                    if "tgif" in train_file['dataset_name'].lower():
                        video_only_dataset_kwargs_train.update({
                ...
                every_single_dataset.append(dataset)
                datasets.append(dataset)
            dataset = ConcatDataset(datasets)
            train_datasets.append(dataset)
        return train_datasets, every_single_dataset
