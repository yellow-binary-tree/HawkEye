...
def setup_main():
    ...
    config.model.num_frame_tokens = 0
    for dataset_cfg in config.train_file:
        if dataset_cfg.get("grounding_method", None) == "frame_token":
            logger.info("dataset %s is using frame_token as grounding method, add %d special frame tokens to the model" % (dataset_cfg['dataset_name'], config.num_frames))
            config.model.num_frame_tokens = config.num_frames
