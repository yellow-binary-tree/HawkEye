...
def setup_model(
    ...
    if config.model.get('model_parallel', False):
        model.set_device_ids([int(os.environ["LOCAL_RANK"]), int(os.environ["LOCAL_RANK"]) + torch.distributed.get_world_size()])
    else:
        model.set_device_ids([int(os.environ["LOCAL_RANK"])])

    model_without_ddp = model
    if config.distributed:
        model = torch.nn.parallel.DistributedDataParallel(
            model,
            device_ids=[config.gpu] if not config.model.get('model_parallel', False) else None,
